...
export const tokenize = (tokens: RegExpToken[]): RegExp[] => {
  ...
  return tokens
    ...
    .map(xs => {
      ...
      const option = xs.filter(x => x.kind === TokenKind[TokenKind.RegexLiteralOption]).map(x => x.value).join("")
      return new RegExp(body, option)
    })
}

export const extractRegex = (s: string): RegExp[] | null => tokenize(lexer(s))
